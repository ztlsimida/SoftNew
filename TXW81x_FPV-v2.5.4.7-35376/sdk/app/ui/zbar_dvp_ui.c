...
static void exit_zbar_ui(lv_event_t * e)
{
    ...
    if(c == 'q')
    {
        ...
        ui_s->now_group = NULL;
        close_stream(ui_s->scale3_stream);
...
static void enter_zbar_ui(lv_event_t * e)
{
    ...
    ui_s->scale3_stream = scale3_stream_not_bind(S_PREVIEW_SCALE3,640,480,320,240,YUV_P0|YUV_OTHER);
    streamSrc_bind_streamDest(ui_s->scale3_stream,R_VIDEO_P0);
    streamSrc_bind_streamDest(ui_s->scale3_stream,R_ZBAR);
    enable_stream(ui_s->scale3_stream,1);
    //启动二维码的流
...
lv_obj_t *zbar_dvp_ui(lv_group_t *group,lv_obj_t *base_ui)
{
    ...
    lv_obj_t *btn =  lv_list_add_btn(base_ui, NULL, "zbar_from_dvp");
