...
static void enter_preview_encode_takephoto_ui(lv_event_t * e)
{
    ...
    ui_s->s = scale3_stream_not_bind(S_PREVIEW_ENCODE_VPP,640,480,320,240,YUV_P0);
    //绑定流到Video P0显示
    streamSrc_bind_streamDest(ui_s->s,R_VIDEO_P0);
    enable_stream(ui_s->s,1);

    //创建jpg流,设置从vpp取数据,所以是1
    ui_s->jpg_s = new_video_app_stream_with_mode(S_JPEG,1,0,0);
    //给到图传测试使用
    streamSrc_bind_streamDest(ui_s->jpg_s,R_RTP_JPEG);
    streamSrc_bind_streamDest(ui_s->jpg_s,R_LVGL_TAKEPHOTO);
